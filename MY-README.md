...
- Configure Agents: Created MD files in `.github` folder that has been used primarily to generate the base code
- MCP Servers : GitHub
...
- Manual: Created a local repository inside project folder, with master as initial branch using command `git init -b master`
- Manual: Added all files to be commited with command `git add --all`
- Manual: Committed the files using command `git commit -m <commit message>`
- Manual: Open MCPRegistry, search for `github-mcp-server` and install it and authenticate MCPServerPlugin to access Github.
- Manual: Install Github CLI from Url `https://cli.github.com/`. Login to github via CLI using command `gh auth login`.Once CLI is authorized in github, Github-Mcp-Server automatically gets authorized to perform actions on your behalf on the remote repo on github.
...
- Manual: Push the changes committed in the local to the new repository using command `git push -u origin master` as MCPServer does not allow you to pass credentials in prompts. 
- Manual: Verify the push on GitHub.


## Deploying app: 
- Running the app locally: Build the application and run it with command: `mvn spring-boot:run 	-Dspring-boot.run.profiles=dev`
- Running on Tomcat: Update `catalina.bat` with the line `set "JAVA_OPTS=	%JAVA_OPTS	% -Dspring.profiles.active=test"`
- Copy the H2 database file to a folder of your choice and update `application-test.properties` with that file path in the Database Url property
- Running on Azure ContainerApp: The `Dockerfile` has been updated to use the `prod` profile and thus it will pick up `application-prod.properties`
